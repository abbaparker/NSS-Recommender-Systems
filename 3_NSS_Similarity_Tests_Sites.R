...
setwd("")
sitedata <- read.csv("Site_Metadata_SpeciesLevel_noCEE_190925.csv", row.names = 1)
#sitedata$DB_Assemblage_ID <- gsub(",", ";", sitedata$DB_Assemblage_ID)
# Input occurrence similarity
raw <- read.csv("Species_PA_noCEE_19Sep25.csv")
#raw$X <- gsub(",", ";", raw$X)

colnames(sitedata)
bins <- unique(sitedata$time_bins2) #find all time bins
bins
sitedata %>% count(time_bins2) #how many sites per time bin

# select sites in each time bin, for original occurrence matrix
thisbin1 <- sitedata[sitedata$time_bins2 %in% bins[7],]  
p5_occ <- raw %>% filter(X %in% thisbin1$DB_Assemblage_ID)
thisbin2 <- sitedata[sitedata$time_bins2 %in% bins[1],]  
f5_occ <- raw %>% filter(X %in% thisbin2$DB_Assemblage_ID)
thisbin3 <- sitedata[sitedata$time_bins2 %in% bins[2],]  
f7_occ <- raw %>% filter(X %in% thisbin3$DB_Assemblage_ID)
thisbin4 <- sitedata[sitedata$time_bins2 %in% bins[3],]  
f9_occ <- raw %>% filter(X %in% thisbin4$DB_Assemblage_ID)
thisbin5 <- sitedata[sitedata$time_bins2 %in% bins[4],]  
f11_occ <- raw %>% filter(X %in% thisbin5$DB_Assemblage_ID)
thisbin6 <- sitedata[sitedata$time_bins2 %in% bins[5],]  
f13_occ <- raw %>% filter(X %in% thisbin6$DB_Assemblage_ID)
thisbin7 <- sitedata[sitedata$time_bins2 %in% bins[6],]  
f15_occ <- raw %>% filter(X %in% thisbin7$DB_Assemblage_ID)
thisbin8 <- sitedata[sitedata$time_bins2 %in% bins[8],]  
...
df_list[[o]]$Site1 <- as.character(df_list[[o]]$Site1)
df_list[[o]]$Site2 <- as.character(df_list[[o]]$Site2)
for (i in 1:nrow(df_list[[o]])) {
  ...
  reg_site1 <- sitedata[sitedata$DB_Assemblage_ID == site1, 5]  #what region is site in, check column numbers
  reg_site2 <- sitedata[sitedata$DB_Assemblage_ID == site2, 5]
  urb_site1 <- sitedata[sitedata$DB_Assemblage_ID == site1,8] #Urban or not
  urb_site2 <- sitedata[sitedata$DB_Assemblage_ID == site2,8]
  count_site1 <- sitedata[sitedata$DB_Assemblage_ID == site1,4] #country name
  count_site2 <- sitedata[sitedata$DB_Assemblage_ID == site2,4]
 ...
 if (urb_site1 == T && urb_site2== T){
      ...
      else if (reg_site1 == "Britain & Ireland" && reg_site2 == "Britain & Ireland"){
      ...
      else if (reg_site1 == "Britain & Ireland" && reg_site2 == "Western Europe" |
               reg_site1 == "Western Europe" && reg_site2 == "Britain & Ireland") {
        df_list[[o]][i,4] <- "BothUrban_BI_WE"
      }
      else if (reg_site1 == "Scandinavia" && reg_site2 == "Britain & Ireland" |
               reg_site1 == "Britain & Ireland" && reg_site2 == "Scandinavia") {
    ...
    if (urb_site1 == T && urb_site2== F){
      if (reg_site1== "Britain & Ireland"){
        if (reg_site2 == "Britain & Ireland"){
      ...
      if (reg_site1== "Western Europe"){
          ...
          else if (reg_site2 == "Britain & Ireland"){
    ...
    if (urb_site1 == F && urb_site2== T){
      ...
      if (reg_site2== "Western Europe"){
        ...
        else if (reg_site1 == "Britain & Ireland"){
          df_list[[o]][i,4] <- "UrbanWE_RuralBI"
        }
      }
      if (reg_site2== "Scandinavia"){
...
print(df_list[[8]] %>% count(UrbanRural))  
print(df_list[[8]] %>% count(UrbanRural_noreg))

#fill in file path where you want to save
save(df_list, file = file.path("SiteBin_df_list_Sep25.Rdata"))
load("SiteBin_df_list_Sep25.Rdata")
...
save(values_lists, file = file.path("ShuffledSitesValues_Lists_Sep25.Rdata")) #input beginning of path for your wd
load("ShuffledSitesValues_Lists_Sep25.Rdata")


# Calculate the mean and standard deviation for each (Site1, Site2) pair from null and compare
for (l in 1:8){
...
save(df_list, file = file.path("SiteBin_Species_df_list_Sep25.Rdata"))
load("SiteBin_Species_df_list_Sep25.Rdata")
...
#save results
write.csv(df_sig, "Sep25Results_PairwiseCorr_Sites_500null_8timebins.csv")
write.csv(Significance_Grouped, "Sep25_SummaryResults_Significance_SitePairs_Species_500null_8timebins.csv")
write.csv(Significance_Grouped_nr, "Sep25_SummaryResults_Significance_NoReg_SitePairs_Species_500null_8timebins.csv")


# Plotting ####
# Plot across bins, broken down by pair urban/rural comparison
Significance_Grouped <- read.csv("Sep25_SummaryResults_Significance_SitePairs_Species_500null_8timebins.csv")
Significance_Grouped_nr <- read.csv("Sep25_SummaryResults_Significance_NoReg_SitePairs_Species_500null_8timebins.csv")
...
# remove comparisons of specific regions
...
Significance_Grouped_nr_group <- Significance_Grouped_nr_group %>% filter(!(UrbRur=="Other"))
...
ggplot(Significance_Grouped_nr_group, aes(x = TimeBin, y = Average_Percentage, color = UrbRur, 
  ...
  labs(
    ...
    color = "Pairwise Urban/Rural Comparison") + ylim(0.35,0.9)+
  theme_bw()

# Comparisons between groups, plots
Significance_Grouped_bothurb <-  Significance_Grouped %>% 
  filter(str_detect(UrbanRural, "BothUrban")) 
write.csv(Significance_Grouped_bothurb, "SitePairs_SignificanceGrouped_Urban.csv") #editing out INF, interpolating empty bins
Significance_Grouped_bothurb <- read.csv("SitePairs_SignificanceGrouped_Urban.csv") #editing out INF, interpolating empty bins

ggplot(Significance_Grouped_bothurb) +
  geom_line(aes(x = TimeBin, y = Percentage_Interpolated, color = UrbanRural, group = UrbanRural), size = 1) +
  geom_point(aes(x = TimeBin, y = Percentage, color = UrbanRural, group = UrbanRural), size = 2) +
  ...
  labs(
    x = "Bin Number",
    y = "Proportion Significant",
    color = "Pairwise Urban/Rural Comparison") + ylim(0.1,1)+
  theme_bw()
...
BI_urban <-  ggplot(Significance_Grouped_BI_urb, aes(x = TimeBin, y = Percentage, color = UrbanRural, group = UrbanRural)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  ...
  labs(
    x = "Bin Number",
    y = "Proportion Significant",
    color = "Pairwise Urban/Rural Comparison") + ylim(0.1,1)+ labs(x=NULL)+
  theme_bw()
...
BI_rural <- ggplot(Significance_Grouped_BI_rur, aes(x = TimeBin, y = Percentage, color = UrbanRural, group = UrbanRural)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_color_manual(values = c("BothUrban_BI" = "blue2", "BothUrbanWE" = "red",
                                "BothUrban_SC" = "#FFD700",
...
WE_urb <- ggplot(Significance_Grouped_WE_urb) +
  geom_line(aes(x = TimeBin, y = Percentage_Interpolated, color = UrbanRural, group = UrbanRural), size = 1) +
  geom_point(aes(x = TimeBin, y = Percentage, color = UrbanRural, group = UrbanRural),size = 2) +
  scale_color_manual(values = c("BothUrban_BI" = "blue2", "BothUrban_WE" = "red",
                                "BothUrban_SC" = "#FFD700",
...
WE_rural <- ggplot(Significance_Grouped_WE_rur, aes(x = TimeBin, y = Percentage, color = UrbanRural, group = UrbanRural)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_color_manual(values = c("BothUrban_BI" = "blue2", "BothUrban_WE" = "red",
                                "BothUrban_SC" = "#FFD700",
...
SC_urb <- ggplot(Significance_Grouped_SC_urb, aes(x = TimeBin, y = Percentage, color = UrbanRural, group = UrbanRural)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_color_manual(values = c("BothUrban_BI" = "blue2", "BothUrban_WE" = "red",
                                "BothUrban_SC" = "#FFD700",
...
SC_rur <- ggplot(Significance_Grouped_SC_rur, aes(x = TimeBin, y = Percentage, color = UrbanRural, group = UrbanRural)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_color_manual(values = c("BothUrban_BI" = "blue2", "BothUrban_WE" = "red",
                                "BothUrban_SC" = "#FFD700",
