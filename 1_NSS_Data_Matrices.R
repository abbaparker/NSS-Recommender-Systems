...
#Read in data
d1 <- read_excel("SupplementaryTable6a_NSS_FamilyData_Feb2025_withCEE_update.xlsx")
colnames(d1)
# counts per region
...
assemblage <- d1 %>% dplyr::select("DB_Assemblage_ID", "Site_name", "Settlement_modern_name",
                            ...
                            "Start_date_CE", "End_date_CE", "Earliest_relevant_urban_date_in_NSS",    
                            "Decimal_Latitude", "Decimal_Longitude","Time.mid") 

fassemblage <-  unique(assemblage)  
write.csv(fassemblage, "Site_Metadata_FamilyLevel_noCEE_180224.csv")
assemblage <-  unique(assemblage)
write.csv(assemblage, "Site_Metadata_SpeciesLevel_noCEE_180225.csv")
...
write.csv(data_species, "TraitData_Species_NSS_18Feb25.csv")

# Build site/occurrence matrices ####

#choose one of family or species level, read above saved files 
straits <- read.csv("TraitData_Families_NSS_19Feb25.csv")
taxa <- taxa  #species
taxa <- ftaxa #genera

# select site names
sites <- read.csv("Site_Metadata_FamilyLevel_noCEE_220225.csv", row.names = 1) #or SpeciesLevel
site <- unique(sites$DB_Assemblage_ID) 

#make NISP numeric
d2$NISP <- as.numeric(d2$NISP)
...
for (q in 1:nrow(sites)){  #row for each locality
  ...
  for (r in 1:length(taxa)){  #column for each taxon
    ...
    records <- pt %>% filter(GBIF_species==taxa[r])# %>% filter(ID %in% pt$ID)
...
write.csv(Occ_matrix, "Species_PA_CEE_18Feb25.csv")
write.csv(Nisp_matrix, "Species_NISP_CEE_18Feb25.csv")
...
data_all <- read.csv("Family_PA_Feb25.csv", header = TRUE)
...
colnames(d2)
sieveS <- unique(d2$DB_Assemblage_ID[which(d2$DB_sieved==T)]) #list of sites that have been sieved
length(sieveS)/length(site)  #fraction of sites that are sieved
data_nisp <- read.csv("Family_NISP_noCEE_22Feb25.csv", header = TRUE)
colnames(data_nisp)
sieve_nisp <- as.data.frame(matrix(nrow=length(site), ncol=length(taxa)))
species <- gsub(" ", "_", taxa)
colnames(sieve_nisp) <- species
...
for (g in 1:length(sieveS)){  # for all sieved sites
  ...
  Ndat <- data_nisp[data_nisp$X == sieveS[g],2:53]  #data from the site
...
write.csv(sieve_nisp, "Sieved_Family_NISPfraction_noCEE_22Feb25.csv")
