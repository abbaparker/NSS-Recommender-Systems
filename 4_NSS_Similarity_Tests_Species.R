...
# Set wd where timebin-specific site/occurrence matrices are saved (produced in script 2)
setwd("")
p5_occ <- read.csv("Species_PA_500b.csv", row.names = 1)
f5_occ <- read.csv("Species_PA_500_700.csv", row.names = 1)
f7_occ <- read.csv("Species_PA_700_900.csv", row.names = 1)
f9_occ <- read.csv("Species_PA_900_1100.csv", row.names = 1)
f11_occ <- read.csv("Species_PA_1100_1300.csv", row.names = 1)
f13_occ <- read.csv("Species_PA_1300_1500.csv", row.names = 1)
f15_occ <- read.csv("Species_PA_1500_1700.csv", row.names = 1)
f17_occ <- read.csv("Species_PA_1700a.csv", row.names = 1)
...
low_counts2 <- names(empty_column_counts[empty_column_counts>4]) #remove these 14 species
all_remove <- c(low_counts, low_counts2)
all_remove <- unique(all_remove)  #21 species less than 8 sites total, or missing from more than 4 time bins

p5_occ <- p5_occ[, !colnames(p5_occ) %in% all_remove]  #leaves 78 species
...
for (i in 1:nrow(df)) {
  taxon1 <- df$Taxon1[i]
  taxon2 <- df$Taxon2[i]
  # Get the life history category values for Taxon1 and Taxon2 from the family trait data
...
# fill in file path to save data
save(values_lists, file = file.path("SpeciesBin_values_list_Sep25.Rdata"))
load("SpeciesBin_values_list_Sep25.Rdata")
...
# Plot across bins, broken down by pairwise life history traits
...
colnames(null_res)
null_res[,12:35] <- lapply(null_res[,12:35], as.numeric)
